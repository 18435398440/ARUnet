...
from U_resnet_bottleneck import ARUnet
...
def predict(unet, number = [], image_path='./data/test1/', image=None):
    ...
    if image:
        ...
        for i in number:
            ...
            cv2.imwrite('./data/segnet_CFD_result/{}'.format(i), temp)



if __name__ == '__main__':
    unet = get_unet('./models/train/CZ1_train_bottleneck_down+up+45.pkl')
    image_path = './data/test1/'
    number = os.listdir(image_path)
    predict(unet, number, image_path=image_path, image=None)
